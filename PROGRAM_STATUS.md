...
Last verified: `2026-08-29T01:35:49Z`. HEADs are the verified default-branch revisions, not an assumption about later changes.
...
| 17 | [research](https://github.com/opsle/research) | program infrastructure | `953e442753f9` | `PROTOTYPED` | [authoritative 19-repository ledger plus machine-readable 17-concept theory registry, canonical theory map, normative Visible Value Contract, JSON schema, semantic validators, dogfood/interoperability evidence, deterministic status renderer, lifecycle validator, operating rules, mutation tests, and integrity CI](program/THEORY_MAP.md); 40 of 40 schema/claim-ceiling/repository/theory-registry/theory-map/renderer tests and both captured receipt validations passed locally; prior PR #5 integrity CI passed and the current reconciliation PR remains subject to CI | Agent Gearbox has no public canonical repository, repository creation requires a separately authorized run, and the program has no canonical measured concept experiment. | In a separately authorized documentation/specification-only run, create and register opsle/gearbox at THEORY without implementing Gearbox or launching model/provider subjects. | — | active |
...
Concept coverage: 17 canonical concepts; 16 current concept repositories mapped exactly once; Agent Gearbox has no current repository.

Canonical map: `program/THEORY_MAP.md`. Machine registry: `program/theory-registry.json`.

## Highest-priority workstream

Publish the canonical Agent Gearbox theory in a separately authorized opsle/gearbox repository without implementation, provider/model execution, or repository consolidation; then resume the independent EXP-001 offline benchmark freeze.
...
Create and register opsle/gearbox in a separately authorized documentation/specification-only run, preserving the 19-repository history and adding the new repository as a twentieth tracked entry; publish the minimal primary-developer Gearbox boundary and no runtime implementation.
